...
## measure s1s3
     ...
     while delay ==0:
        continue
     s1s3_delay=delay
     print "s1s3 CALCULATE DELAY: ", delay
     delay=0
     mytimer.cancel()
  ### measure s1s4
     src_dpid=s1s4_src
     ...
     while delay ==0:
        continue
     s1s4_delay=delay
     print "s1s4 CALCULATE DELAY: ", delay
     mytimer.cancel()
     delay=0
   ### get intent
...
def test():
  ...
  global delay,src_dpid, dst_dpid
  ### tutaj tworzenie intentow
  intent=Intent('10.0.0.1','10.0.0.4',50) ## demand podany w ms
  intent1=Intent('10.0.0.1','10.0.0.5',200)
  ## obsluga intentu
